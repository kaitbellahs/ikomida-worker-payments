...
class PaymentsWorker {
  ...
  async processMessages(message: Message, channel: Channel) {
    try {
      ...
      const payload: Types.Classes.CAMQPPayload<string> = Types.Classes.CAMQPPayload.fromObject(
      ...
      if (payload.method === 'cancelPayment') {
        ...
        let i = 0
        const waitTime =
          new Date().getTime() - 5 * 60 * 1000 < userPaymentModel.createdAt.getTime() ? 30 * 1000 : 5 * 1000
        for (i = 1; i <= 5; i++) {
          const cancelChargeObject = Types.Classes.Pagseguro.CPagSeguroCreateCharge.init(
            '',
        ...
        this.logger.error(
          `nao foi possivel cancelar cobranca após ${i} tentativas em ${(startTime - new Date().getTime()) / 1000}s.`
        )
      }
    } catch (error: any) {
