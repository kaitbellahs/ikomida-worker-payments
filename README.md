...
Handles payment work that must not block an HTTP request — provider calls that are slow, retryable, or triggered by a webhook rather than a user.

## Queue

Consumes `PAYMENT_QUEUE` from RabbitMQ. Messages are processed with bounded retries and acknowledged only on success; failures are negatively acknowledged so nothing is lost silently.
